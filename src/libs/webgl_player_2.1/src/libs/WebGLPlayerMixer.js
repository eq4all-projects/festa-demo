...
class WebGLMixer {
  constructor(characterObject, boneArray, loadingWorkerPath, system) {
    ...
    if (this.workerEnabled) {
      try {
        ...
        console.log("✅ WebGL Worker 로드 성공");
      } catch (error) {
        console.error("❌ WebGL Worker 로드 실패:", error);
        console.log("⚠️ Worker를 비활성화하고 메인 스레드에서 처리합니다");
        this.workerEnabled = false;
        this.loadingWorker = null;

        // Worker 로드 실패 시 오류 상태 설정
        this.playerStatus = 5;
      }
    } else {
      this.loadingWorker = null;
  ...
  async setSLTypeMetadata(id, version, level, dynamicVariables) {
    ...
    if (version === 0.3) {
      ...
      calledData = await this.fingerSpells._3_SetFingerSpellAttribute(
        calledData,
        dynamicVariables
      );
      // console.log(calledData);

      calledData = await this.fingerSpells._3_SetFingerSpellAniData(
        calledData,
        this.additiveAnimationsFile
      );
    } else if (version === 0.4) {
      if (level === "Sentence") {
        ...
        let sentence = await apiDispatcher(
          this.config.APIOrigin + this.config.getSentenceInfoAPI(id),
          "GET"
        );
      ...
      calledData = await this.fingerSpells._4_SetFingerSpellAttribute(
        calledData,
        dynamicVariables
      );
      calledData = await this.fingerSpells._4_SetFingerSpellAniData(
        calledData,
        this.additiveAnimationsFile
      );
    }
  ...
  async eq4FileToClipStructure(file, index) {
    ...
    await Promise.all(
      _.remove(
        ...
        function (track) {
          if (track.type == "Morph") {
            if (
              this.meshFace.morphTargetDictionary[track.morphName] !== undefined
            ) {
              return false;
            } else {
              return true;
    ...
    if (
      !rightShoulder1Position &&
      !rightShoulder1Quaternion &&
      !rightShoulder1Scale
    ) {
    } else {
      ...
      _.map(
        ...
        async function (track) {
            ...
            if (!this.isNumber(this.animations[index].Speed)) {
              if (typeof this.animations[index].Speed == "object") {
                if (!this.isNumber(this.animations[index].Speed[0])) {
                  ...
                  this.animations[index].Speed =
                    this.animations[index].Speed[0];
                }
              } else {
                this.animations[index].Speed = 1;
              }
          ...
          if (track.type == "Morph") {
            ...
            track.type = "Number";
            track.name =
              track.name + ".morphTargetInfluences[" + morphNum + "]";
          }
          ...
          if (track.name.search(".position") > -1) {
            ...
            if (
              this.retargetInfo.position[name] === undefined &&
              name !== "Bip001"
            ) {
              return;
            }
    ...
    await Promise.all(
      _.map(
        ...
        async function (animation, index) {
          ...
          if (version == 0.3) {
            let filepathIndex = await this.searchStringInArray(
              animation.animationName + ".eq4",
              tmpFileList
            );
            tmpFileList[filepathIndex] = tmpFileList[filepathIndex].replace(
              "models/",
              ""
            );
            ...
            promise[index] = apiDispatcher(
              ...
              null,
              {
                filepath: this.config.getEq4Filepath(
                  tmpFileList[filepathIndex]
                ),
              },
              this.system
            );
            ...
            promise[index] = apiDispatcher(
              this.config.fileOrigin +
                this.config.getEq4FileAPI() +
                this.config.getEq4Filepath(animation),
              "GET",
              null,
              null,
              this.system
  ...
  async setOneAnimationFileStructure() {
    this.tmpOneAnimationFile = await this.eq4FileToClipStructure(
      this.oneAnimationEq4File
    );
  }
  ...
  async forceIdlePlay(adjustBones, weight) {
    ...
    _.map(
      ...
      function (track) {
        ...
        let exist = _.some(adjustBones, (value) =>
          _.includes(nameArray[0], value)
        );
        ...
        if (nameArray[1] === "position") {
          ...
          if (
            this.boneArray["Mesh_Face"].morphTargetDictionary["REye_Close"] ==
              morphIndex ||
            this.boneArray["Mesh_Face"].morphTargetDictionary["LEye_Close"] ==
              morphIndex ||
            this.boneArray["Mesh_Face"].morphTargetDictionary[
              "REyeBrow_Lower"
            ] == morphIndex ||
            this.boneArray["Mesh_Face"].morphTargetDictionary[
              "LEyeBrow_Lower"
          ...
          if (track.values[Math.ceil(this.idleIndex / 2)] !== undefined) {
            (this.boneArray["Mesh_Face"].morphTargetInfluences[
              parseInt(morphIndex)
    ...
    _.map(
      ...
      function (track) {
        ...
        if (nameArray[1] === "position") {
          ...
          this.boneArray[nameArray[0]].quaternion.slerp(
            new THREE.Quaternion(
              track.quaternionValues[
                Math.ceil(this.addedAnimationIndex / 2) * 4
              ],
              track.quaternionValues[
                Math.ceil(this.addedAnimationIndex / 2) * 4 + 1
              ],
              track.quaternionValues[
                Math.ceil(this.addedAnimationIndex / 2) * 4 + 2
              ],
              track.quaternionValues[
                Math.ceil(this.addedAnimationIndex / 2) * 4 + 3
              ]
            ),
            weight
          );
          ...
          if (
            track.values[Math.ceil(this.addedAnimationIndex / 2)] !== undefined
          ) {
            (this.boneArray["Mesh_Face"].morphTargetInfluences[
              parseInt(morphIndex)
    ...
    _.map(
      ...
      function (track) {
        if (track.name == "Mesh_Face") {
          let morphIndex =
            this.boneArray["Mesh_Face"].morphTargetDictionary[track.morphName];
  ...
  async playWithBlendingOnce() {
    ...
    if (this.workerEnabled && this.loadingWorker) {
      ...
      // 워커 오류 처리
      this.loadingWorker.onerror = (error) => {
        console.error("❌ WebGL Worker 오류:", error);
        console.log("⚠️ Worker를 비활성화하고 메인 스레드에서 처리합니다");
        this.workerEnabled = false;
        this.loadingWorker = null;
      };
    } else {
      console.log("⚠️ WebGL Worker가 비활성화되어 있습니다");
    }
  }
  ...
  async actionSetWeight(kinematics) {
    ...
    try {
      ...
      if (
        this.actionPlaying ||
        this.actionPlayingBlender ||
        (!this.actionPlayingBlender && this.currentPlayTime > 0)
      ) {
        ...
        if (
          this.repeatEndTime !== 0 &&
          (this.repeatStartTime > this.currentPlayTime ||
            this.repeatEndTime <= this.currentPlayTime)
        ) {
        ...
        if (this.actionPlayingBlender) {
          if (
            this.blendings[this.blendings.length - 1].trackEndTime <=
          ...
          _.map(
            ...
            function (blending) {
              if (
                ...
                if (
                  blending.fadeInIndex !== null &&
                  blending.fadeOutIndex !== null
                ) {
                  ...
                  if (blending.fadeOutIndex !== null) {
                    ...
                    blendingWeight =
                      1 -
                      (this.currentPlayTime - blending.trackStartTime) / 0.3;
          ...
          _.map(
            ...
            function (animation, index) {
              ...
              if (
                ...
                this.actions[index].time =
                  this.currentPlayTime - animation.parentStartTime;
                ...
                if (blendingBlock !== null) {
                  if (index === blendingBlock.fadeInIndex) {
                    this.actions[index].setEffectiveWeight(
                    ...
                    if (animation.isFingerspell) {
                      ...
                      if (
                        Object.keys(kinematics.playedAnimations).length == 1
                      ) {
                ...
                if (this.actionPlaying) {
                  if (
                    index % this.maxLoadingAnimation === 1 &&
                    !animation.played
                  ) {
                    this.loadingCount++;
                      ...
                      try {
                        this.loadingWorker.postMessage({
                          ...
                          data: {
                            ...
                            morphDictionary:
                              this.meshFace.morphTargetDictionary,
          ...
          if (this.captureEnable) {
            ...
            if (this.captureDownloadAuto) {
              this.capturer.save(async (blob) => {
                const formData = new FormData();
                formData.append(
                  this.captureFilename.split(".")[0] + "_capture.tar",
                  blob
                );
      ...
      console.error("WebGL Player Error:", e);
  ...
  async setCurrentPlayTimeBlender(currentTime, kinematics) {
            ...
            if (
              ...
              if (
                ...
                if (blending.fadeOutIndex !== null) {
                  ...
                  blendingWeight =
                    1 - (this.currentPlayTime - blending.trackStartTime) / 0.3;
    ...
    // 애니메이션 총 Weight가 1이 되지 않으면 남은 Weight는 Idle Action으로 채운다.
    if (totalWeight <= 1 && !this.actionPlayingBlender)
      this.idleAction.setEffectiveWeight(1 - totalWeight);
  ...
  async saveSentenceBlendingInfo() {
    let response = await apiDispatcher(
      this.config.APIOrigin +
        this.config.getSentenceAPI(this.sentenceInfo.sentence_id),
      "PUT",
      this.sentenceInfo
    );
  ...
  async initialDisconnectPanel(container) {
    ...
    createdPanel.style["height"] = !!container.style.height
      ? container.style.height
      : "100%";
    createdPanel.style["width"] = !!container.style.width
