import {
    ...
    Matrix4,
    Mesh, MeshBasicMaterial, Object3D, PlaneGeometry, Quaternion,
    Vector3
} from 'three';
...
class IKConstraintsHelper extends LineSegments
{
    constructor(object, constraints)
    {
        ...
        for (let i = 0; i < nbConstraints; ++i)
        {
            let l = linkConstraints[i];
            if (l.enabled === false || !l.limitation) continue;
            let geometry1 = new PlaneGeometry(planeSize, planeSize);
            let geometry2 = new PlaneGeometry(planeSize, planeSize);
