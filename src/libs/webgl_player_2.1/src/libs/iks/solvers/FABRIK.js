...
)
{
    ...
    let root = chain[constraints.indexes[0]].parent;
    ...
    if (copyFromChain)
    {
        ...
        {
            let link = chain[constraints.indexes[i]];
...
)
{
    ...
    let nv1 = new Vector3();
    let nv2 = new Vector3();
    for (let i = 0; i < constraints.indexes.length - 1; ++i) {
        let currentBone = chain[constraints.indexes[i]];
        let currentStart = proxies[i];
        let currentEnd = proxies[i + 1];

        // last bone’s unit vector
        if (i === 0) currentV1.set(0, 1, 0);
        else currentV1.copy(currentV2);
        ...
        if (i === 0) {
            ...
            // work in local world
            let lastBone = chain[constraints.indexes[i - 1]];
...
)
{
    ...
    const endIndex = constraints.indexes.length - 1;
...
)
{
    ...
    const chainLength = constraints.indexes.length - 1; // end bone === end effector in our setup
    ...
    for (let loop = chainLength - 1; loop >= 0; --loop)
    {
        ...
        if (loop !== chainLength - 1)
        {
            ...
            else if (thisBoneJointType === JointType.LOCAL_HINGE)
            {
                ...
                if (loop > 0)
                {
                    this.computeLocalTransform(localTransform, loop, chain, constraints);
...
)
{
    ...
    const proxy = constraints.chainProxy;
    const boneLengths = constraints.boneLengths;
    const chainLength = constraints.indexes.length - 1; // end bone === end effector!
...
FABRIK.prototype.computeLocalTransform = function (localTransform, loop, chain, constraints) {
    let matrixWorldInverse = this.m1;
    let boneMatrix = this.m2;
    const t = this.v11;
    const s = this.v12;
    boneMatrix.multiplyMatrices(matrixWorldInverse, chain[constraints.indexes[loop - 1]].matrixWorld);
